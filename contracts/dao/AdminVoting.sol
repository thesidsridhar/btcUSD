...
/**
    @title BBL DAO Admin Voter
    @notice Primary ownership contract for all BBL contracts. Allows executing
            arbitrary function calls only after a required percentage of BBL
...
contract AdminVoting is DelegatedOps, SystemStart {
    ...
    function createNewProposal(address account, Action[] calldata payload) external callerOrDelegated(account) {
        ...
        // if the only action is `BBLCore.setGuardian()`, use
    ...
    function cancelProposal(uint256 id) external {
        require(msg.sender == BBLCore.guardian(), "Only guardian can cancel proposals");
        require(id < proposalData.length, "Invalid ID");
    ...
    /**
        @dev Unguarded method to allow accepting ownership transfer of `BBLCore`
    ...
    function _isSetGuardianPayload(uint256 payloadLength, Action memory action) internal view returns (bool) {
        if (payloadLength == 1 && action.target == address(BBLCore)) {
